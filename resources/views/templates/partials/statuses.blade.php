<div class="row">
    <div class="media Posts">
        <a class="pull-left" href="{{ route('profile.index', ['id' => $status->user->id]) }}">
            <img class="media-object" alt="{{ $status->user->getName() }}" 
                src="@if(!$status->user->profile_img) {{ URL::to('/') }}/images/users/profile-img.jpg  @else {{ $status->user->profile_img }} @endif" width="60">
        </a>
        <div class="media-body">
            <h4 class="media-heading"><a href="{{ route('profile.index', ['id' => $status->user->id]) }}">{{ $status->user->getName() }}</a></h4>
            <p>{{ $status->body }}</p>
